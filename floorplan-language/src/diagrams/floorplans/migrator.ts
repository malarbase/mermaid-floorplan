...
const MIGRATION_REGISTRY: MigrationStep[] = [
  {
    ...
    description:
      'Migrate door_width/door_height to door_size, window_width/window_height to window_size',
    migrate(content: string): string {
      ...
      migrated = migrated.replace(configRegex, (_match, configContent) => {
        ...
        if (doorWidth && doorHeight && doorWidthMatch && doorHeightMatch) {
          ...
          changes.push(
            `Migrated door_width and door_height to door_size: (${doorWidth} x ${doorHeight})`,
          );
          ...
          if (doorWidth && doorWidthMatch) {
            ...
            changes.push(
              `Migrated door_width to door_size (used default height: ${defaultHeight})`,
            );
          }
        ...
        if (windowWidth && windowHeight && windowWidthMatch && windowHeightMatch) {
          ...
          changes.push(
            `Migrated window_width and window_height to window_size: (${windowWidth} x ${windowHeight})`,
          );
          ...
          if (windowWidth && windowWidthMatch) {
            ...
            changes.push(
              `Migrated window_width to window_size (used default height: ${defaultHeight})`,
            );
          }
          if (windowHeight && windowHeightMatch) {
            ...
            changes.push(
              `Migrated window_height to window_size (used default width: ${defaultWidth})`,
            );
          }
        }
...
function findMigrationPath(from: SemanticVersion, to: SemanticVersion): MigrationStep[] {
  ...
  for (const step of MIGRATION_REGISTRY) {
    ...
    if (compareVersions(step.fromVersion, from) >= 0 && compareVersions(step.toVersion, to) <= 0) {
      path.push(step);
    }
  }
...
export function migrate(
  ...
  try {
    ...
    const target = typeof targetVersion === 'string' ? parseVersion(targetVersion) : targetVersion;

    // Extract current version from content
    const currentVersionStr = extractVersionFromContent(content);
    const current = currentVersionStr ? parseVersion(currentVersionStr) : parseVersion('1.0.0'); // Assume 1.0.0 if no version
    ...
    if (compareVersions(current, target) > 0) {
      result.errors.push(
        `Cannot downgrade from ${formatVersion(current)} to ${formatVersion(target)}.`,
      );
      return result;
    }
    ...
    if (migrationPath.length === 0) {
      result.errors.push(
        `No migration path found from ${formatVersion(current)} to ${formatVersion(target)}.`,
      );
      return result;
    }
...
function updateVersionInContent(content: string, newVersion: string): string {
  ...
  content = content.replace(/%%\{\s*version\s*:\s*[0-9.]+\s*\}%%/, `%%{version: ${newVersion}}%%`);
  ...
  if (
    !content.match(/%%\{\s*version\s*:\s*[0-9.]+\s*\}%%/) &&
    !content.match(/^---\s*\n[\s\S]*?version\s*:/m)
  ) {
