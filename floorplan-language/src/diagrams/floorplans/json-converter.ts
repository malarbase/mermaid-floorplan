...
import { CURRENT_VERSION, extractVersionFromAST } from './version-resolver.js';
  ...
  for (const style of floorplan.styles) {
    ...
    for (const prop of style.properties) {
      ...
      if (prop.stringValue) {
        ...
        (jsonStyle as unknown as Record<string, string | number>)[key] = prop.stringValue.replace(
          /^["']|["']$/g,
          '',
        );
...
function convertStairShape(
  ...
  const normalizeLanding = (
    landing: { width: ValueWithUnit; height: ValueWithUnit } | undefined,
  ...
  if (isSpiralStair(shape)) {
    return {
      ...
      outerRadius:
        normalizeValueToDefaultUnit(shape.outerRadius, defaultUnit) ?? shape.outerRadius.value,
  ...
  if (isSegmentedStair(shape)) {
    return {
      ...
      segments: shape.segments.map((seg) =>
        convertStairSegment(seg, roomBoundsMap, stairWidth, defaultUnit),
      ),
    };
  }
  ...
  if (isTurnSegment(segment)) {
    return {
      ...
      landing: segment.landing
        ? [
            normalizeValueToDefaultUnit(segment.landing.width, defaultUnit) ??
              segment.landing.width.value,
            normalizeValueToDefaultUnit(segment.landing.height, defaultUnit) ??
              segment.landing.height.value,
          ]
        : undefined,
 ...
 */
function convertStairMaterial(material: { properties: Array<{ name: string; value: string }> }): {
  [key: string]: string;
} {
