...
import {
  ...
  isDeprecated,
  isRemoved,
} from './diagrams/floorplans/deprecation-registry.js';
import { resolveFloorPositions } from './diagrams/floorplans/position-resolver.js';
import {
  getAvailableThemes,
  isValidTheme,
  normalizeConfigKey,
} from './diagrams/floorplans/styles.js';
import {
  hasMixedUnitSystems,
  type LengthUnit,
  toMeters,
  VALID_UNITS,
} from './diagrams/floorplans/unit-utils.js';
...
export class FloorplansValidator {
  ...
  private extractConnections(floorplan: Floorplan): SimpleConnection[] {
    ...
    for (const connection of floorplan.connections) {
      ...
      const doorWidthPercent =
        connection.doorType === 'double-door' ? DOUBLE_DOOR_WIDTH_PERCENT : DOOR_WIDTH_PERCENT;
  ...
  private isBidirectional(c1: SimpleConnection, c2: SimpleConnection): boolean {
    return (
      (c1.fromRoom === c2.toRoom && c1.toRoom === c2.fromRoom) ||
  ...
  checkDuplicateStyleNames(floorplan: Floorplan, accept: ValidationAcceptor): void {
    ...
    for (const style of floorplan.styles) {
      ...
      if (existing) {
        accept('error', `Duplicate style name '${style.name}'. Style names must be unique.`, {
          node: style,
          property: 'name',
        });
      } else {
   ...
   */
  private computeRoomBounds(
    rooms: Room[],
    floorplan: Floorplan,
    floor?: Floor,
  ): Map<string, { x: number; z: number; width: number; height: number }> {
  ...
  private hasOpeningConnection(
    ...
    for (const conn of floorplan.connections) {
      ...
      const matchesAB =
        fromRoom === roomA && fromWall === wallA && toRoom === roomB && toWall === wallB;
      const matchesBA =
        fromRoom === roomB && fromWall === wallB && toRoom === roomA && toWall === wallA;
  ...
  checkStylePropertyValue(property: StyleProperty, accept: ValidationAcceptor): void {
    ...
    if (numericProps.includes(property.name)) {
      if (property.numberValue !== undefined) {
        ...
        accept('error', `${property.name} must be a number between 0 and 1, not a string.`, {
          node: property,
  ...
  checkConfigPropertyValue(property: ConfigProperty, accept: ValidationAcceptor): void {
    ...
    if (normalizedKey === 'fontSize' && property.value !== undefined) {
      if (property.value <= 0) {
        accept('error', `fontSize must be a positive number, got ${property.value}.`, {
          node: property,
          property: 'value',
        });
      }
    ...
    for (const floor of floorplan.floors) {
      ...
      for (const conn of floorplan.connections) {
        ...
        if (!conn.size.fullHeight && conn.size.height) {
          ...
          const fromRoomHeight = this.getRoomHeight(
            this.findRoomByName(allRooms, fromRoomName)!,
            floorplan,
            floor,
          );
          const toRoomHeight = this.getRoomHeight(
            this.findRoomByName(allRooms, toRoomName)!,
            floorplan,
            floor,
          );
  ...
  checkVerticalConnectionAlignment(floorplan: Floorplan, accept: ValidationAcceptor): void {
    ...
    for (const vc of floorplan.verticalConnections) {
      ...
      for (let i = 0; i < links.length - 1; i++) {
        ...
        const tolerance = 0.5;
        if (
          Math.abs(currentPos.x - nextPos.x) > tolerance ||
  ...
  checkCirculationOutsideRoomBounds(floorplan: Floorplan, accept: ValidationAcceptor): void {
    ...
    for (const floor of floorplan.floors) {
      ...
      for (const stair of floor.stairs) {
        ...
        const containingRoom = roomBounds.find(
          (r) => stairX >= r.minX && stairX < r.maxX && stairY >= r.minY && stairY < r.maxY,
        );
      ...
      for (const lift of floor.lifts) {
        ...
        const containingRoom = roomBounds.find(
          (r) => liftX >= r.minX && liftX < r.maxX && liftY >= r.minY && liftY < r.maxY,
        );
        ...
        for (let i = 0; i < segments.length; i++) {
          ...
          if (isTurnSegment(segment) && segment.landing) {
            ...
            const nextFlightWidth =
              flightIndex < flightWidths.length ? flightWidths[flightIndex] : undefined;
