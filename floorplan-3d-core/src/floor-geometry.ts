...
import { getCSG, isCsgAvailable } from './csg-manager.js';
 ...
 */
export function generateFloorSlabs(floor: JsonFloor, options: FloorSlabOptions = {}): THREE.Group {
...
export function computeFloorBounds(floor: JsonFloor): THREE.Box3 {
  ...
  for (const room of floor.rooms) {
    const roomBox = new THREE.Box3(
      ...
      new THREE.Vector3(
        room.x + room.width,
        room.roomHeight ?? DIMENSIONS.WALL.HEIGHT,
        room.z + room.height,
      ),
    );
    box.union(roomBox);
