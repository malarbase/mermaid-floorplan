...
import type { AreaUnit, Floorplan, LengthUnit } from 'floorplan-language';
import {
  buildStyleContext,
...
async function main() {
  ...
  const formats = [options.generateSvg ? 'SVG' : null, options.generatePng ? 'PNG' : null]
    .filter(Boolean)
    .join(' + ');
