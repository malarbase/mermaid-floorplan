...
test.describe('Advanced Mode', () => {
  ...
  test('floor visibility controls work', async ({ page }) => {
    ...
    const floorToggle = floorsSection.locator('input[type="checkbox"]').or(
      floorsSection.locator('button')
    ).first();
...
test.describe('Editor Mode', () => {
  ...
  test('EditorBundle includes all editor components', async ({ page }) => {
    ...
    const selectionControls = page.locator('[data-component="selection-controls"]').or(
      page.locator('button').filter({ hasText: /add|delete|copy|focus/i })
    );
  ...
  test('properties panel exists', async ({ page }) => {
    ...
    const propsPanel = page.locator('[data-component="properties-panel"]').or(
      page.locator('.properties-panel')
    );
