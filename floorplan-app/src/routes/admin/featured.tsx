...
export default function FeaturedProjects() {
  ...
  const handleToggle = async (projectId: Id<"projects">, currentFeatured: boolean, projectName: string) => {
    try {
    ...
    <div class="p-6">
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p class="text-base-content/60 mt-1">Curate high-quality floorplans for the explore page</p>
        </div>
                  ...
                  <tr class="hover:bg-base-200/30 transition-colors">
                    <td class="text-center">
                      <Show when={project.isFeatured}>
                        <span class="text-yellow-500 text-xl" title="Featured">★</span>
                      </Show>
                    </td>
                    <td>
                      <div class="font-bold">{project.displayName || "Untitled Project"}</div>
                      <div class="text-xs text-base-content/60 font-mono mt-0.5">{project.slug}</div>
                    </td>
                    <td>
