...
import { Show, createMemo, ParentComponent } from "solid-js";
        ...
        <div class="flex h-screen bg-base-100 overflow-hidden font-sans">
            <aside class="w-64 bg-base-200 border-r border-neutral flex flex-col flex-none z-20 shadow-lg">
                <div class="h-16 flex items-center px-6 border-b border-neutral bg-base-200/95 backdrop-blur-sm">
                    <A href="/" class="text-xl tracking-wider font-display font-bold text-base-content hover:text-primary transition-colors flex items-center gap-2">
                        FLOORPLAN <span class="text-xs font-normal px-1.5 py-0.5 rounded bg-primary/20 text-primary border border-primary/20 tracking-normal">ADMIN</span>
                    </A>
                </div>
                
                <nav class="flex-1 py-6 flex flex-col gap-1 overflow-y-auto scrollbar-thin">
                    <div class="px-6 mb-2 text-xs font-semibold text-base-content/40 uppercase tracking-wider">Main</div>
                    ...
                    <div class="px-6 mb-2 mt-6 text-xs font-semibold text-base-content/40 uppercase tracking-wider">Content</div>
                    ...
                    <div class="px-6 mb-2 mt-6 text-xs font-semibold text-base-content/40 uppercase tracking-wider">System</div>
                    
                    <A href="/admin/users" class={linkClass("/admin/users")}>
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                        <span>Users</span>
                        <Show when={user()?.isAdmin}>
                           <span class="ml-auto text-[10px] font-bold px-1.5 py-0.5 rounded bg-warning/10 text-warning border border-warning/20">SUPER</span>
                        </Show>
                    </A>
                ...
                <div class="p-4 border-t border-neutral bg-base-200/50">
                     <A href="/dashboard" class="flex items-center gap-3 px-4 py-3 text-sm font-medium text-base-content/70 hover:bg-base-300 hover:text-base-content rounded-md transition-colors w-full group">
                ...
                <header class="h-16 border-b border-neutral bg-base-100/80 backdrop-blur-md flex items-center justify-between px-8 sticky top-0 z-10">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="w-10 h-10 rounded-full bg-base-300 overflow-hidden ring-2 ring-base-200 ring-offset-2 ring-offset-base-100">
                             <Show when={user()?.image} fallback={<div class="w-full h-full flex items-center justify-center bg-primary text-primary-content font-bold">{user()?.name?.charAt(0)}</div>}>
