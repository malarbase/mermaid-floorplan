...
export function PublicProjectCard(props: PublicProjectCardProps) {
  ...
  const ownerQuery = useQuery(api.users.getById, () => ({
    userId: props.project.userId as Id<'users'>,
  }));
  ...
  return (
    <div
      class={`card bg-base-100 shadow-sm hover:shadow-md transition-shadow duration-200 border border-base-200 ${props.class || ''}`}
    >
      {/* Link overlay */}
      <Show when={owner()}>
        <A
      ...
      <div class="card-body p-4 gap-2">
        <h3
          class="card-title text-base font-semibold truncate leading-tight"
