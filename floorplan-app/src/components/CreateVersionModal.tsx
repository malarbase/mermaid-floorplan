    ...
    <Modal
      ...
      description={
        <>
          ...
          <span class="font-mono font-medium">{props.fromVersion || 'main'}</span>. The new version
          will start with the same content and can be edited independently.
        </>
      }
      error={error()}
    >
          ...
          <div class="flex flex-wrap gap-2 mt-2">
            <For each={suggestions()}>
              {(suggestion) => (
                <button
                  type="button"
                  class={`btn btn-sm ${versionName() === suggestion ? 'btn-primary' : 'btn-ghost'}`}
