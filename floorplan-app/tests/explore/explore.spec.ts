...
test.describe("Collection Page", () => {
  ...
  test("shows collection not found for invalid slug", async ({ page }) => {
    ...
    await expect(page.getByText(/Collection Not Found|doesn't exist/i)).toBeVisible({ timeout: 10000 });
  });
});
...
test.describe("Fork-to-Edit Flow", () => {
  ...
  test("fork redirects to editor with success message", async ({ page }) => {
    ...
    if (await forkDialog.count() > 0) {
      ...
      await expect(successIndicator.or(page.locator('[data-editor]')).first()).toBeVisible({ timeout: 5000 });
    } else {
      test.skip();
    }
  });
...
test.describe("View Count", () => {
  test("view count increments when viewing project", async ({ page }) => {
    ...
    const viewCountElement = projectWithViews.locator('span:has(svg) + span, span:has-text("views")');
    const initialViewText = await viewCountElement.textContent();
    const initialViews = initialViewText ? parseInt(initialViewText.replace(/\D/g, "")) : 0;
    ...
    const updatedViews = updatedViewText ? parseInt(updatedViewText.replace(/\D/g, "")) : 0;
