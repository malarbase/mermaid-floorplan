...
import { getRoomSize, type Room } from 'floorplan-language';
...
export function buildRelativeAssignments(
  ...
  const pending = new Set(Array.from(roomBounds.keys()).filter((name) => name !== anchorRoom));
  ...
  for (let i = 0; i < boundsList.length; i++) {
    for (let j = i + 1; j < boundsList.length; j++) {
      if (roomsOverlap(boundsList[i], boundsList[j])) {
        errors.push(`Rooms '${boundsList[i].name}' and '${boundsList[j].name}' overlap`);
      }
    }
  }
