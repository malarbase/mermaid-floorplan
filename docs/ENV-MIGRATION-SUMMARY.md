...
└── .env.local.admin     # Generated by mise run admin:setup
```
...
| `mise.toml` | Updated `admin-setup` to create `.env.local` directly |
| `mise.toml` | Updated `admin-help` with new instructions |
...
- **Admin Testing:** `mise run admin:help`
- **E2E Testing:** [E2E-TESTING-PROMPT.md](./E2E-TESTING-PROMPT.md)
- **mise.toml Targets:** `make help`
