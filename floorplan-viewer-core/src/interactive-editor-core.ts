...
import { MaterialFactory, DIMENSIONS, type ViewerTheme } from 'floorplan-3d-core';
import { FloorplanAppCore, type FloorplanAppCoreOptions, type FloorplanAppCoreEvents, type EventHandler } from './floorplan-app-core.js';
 ...
 */
export interface InteractiveEditorCoreOptions extends Omit<FloorplanAppCoreOptions, 'enableSelection' | 'allowSelectionToggle'> {
...
export class InteractiveEditorCore extends FloorplanAppCore {
  ...
  get entityLocations(): readonly EntityLocation[] { return this._entityLocations; }
  ...
  on<K extends keyof InteractiveEditorCoreEvents>(
    ...
    return super.on(event as keyof FloorplanAppCoreEvents, handler as EventHandler<FloorplanAppCoreEvents[keyof FloorplanAppCoreEvents]>);
  }
  ...
  protected emitEditorEvent<K extends keyof InteractiveEditorCoreEvents>(
    ...
    const handlers = (this as unknown as { eventHandlers: Map<string, Set<EventHandler<unknown>>> }).eventHandlers;
  ...
  public findEntitiesAtLine(line: number): EntityLocation[] {
    return this._entityLocations.filter(e => 
      e.sourceRange && 
      line >= e.sourceRange.startLine && 
      line <= e.sourceRange.endLine
    );
  }
