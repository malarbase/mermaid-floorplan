...
async function parseDSL(input: string): Promise<LangiumDocument<Floorplan>> {
  ...
  const document = services.shared.workspace.LangiumDocumentFactory.fromString<Floorplan>(
    input,
    uri,
  );
 ...
 */
export async function parseFloorplanDSLWithDocument(
  dslContent: string,
): Promise<ParseResultWithDocument> {
