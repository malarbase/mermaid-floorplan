...
- [ ] 6.7 Read `theme` or `darkMode` from parsed floorplan config (deferred - requires DSL parser integration)
- [ ] 6.8 Apply DSL theme to viewer on load (deferred - requires DSL parser integration)
...
- [ ] 9.1 Update DSL reference in `openspec/project.md` with new config keys (deferred - spec updates needed)
